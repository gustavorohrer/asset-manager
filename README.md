...
- `GET /assets` (asset listing with filters, sorting, pagination, computed threat/vulnerability flags, vulnerability counters `high`/`medium`/`total`, and threat counters `high`/`medium`/`low`/`total`)
